...
if (!function_exists('showFinalCost')) {
    function showFinalCost($finalCost)
    {
        if ($finalCost < 0) {
            $result = '<span class="text-warning">' . number_format($finalCost) . '</span>';
        } else {
            $result = '<span class="text-success">+' . number_format($finalCost) . '</span>';
        }
